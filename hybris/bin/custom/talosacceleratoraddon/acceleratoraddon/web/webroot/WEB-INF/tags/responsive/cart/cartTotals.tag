...
<div class="js-cart-totals">
    <div class="cart-totals-left"><spring:theme code="basket.page.totals.subtotal"/></div>
    <div class="cart-totals-right"><ycommerce:testId code="Order_Totals_Subtotal"><format:price priceData="${cartData.subTotal}"/></ycommerce:testId></div>


    <c:if test="${not empty cartData.deliveryCost}">
        <div class="cart-totals-left"><spring:theme code="basket.page.totals.delivery"/></div>
        <div class="cart-totals-right"><format:price priceData="${cartData.deliveryCost}" displayFreeForZero="TRUE"/></div>
     </c:if>


    <c:if test="${cartData.net && cartData.totalTax.value > 0 && showTax}">
        <div class="cart-totals-left"><spring:theme code="basket.page.totals.netTax"/></div>
        <div class="cart-totals-right"><format:price priceData="${cartData.totalTax}"/></div>
    </c:if>
	...
	<c:if test="${cartData.quoteDiscounts.value > 0}">
		<div class="cart-totals-left discount">
			<spring:theme code="basket.page.quote.discounts" />
		</div>
		<div class="cart-totals-right text-right discount">
    ...
    <div class="cart-totals-left grand-total"><spring:theme code="basket.page.totals.total"/></div>
    <div class="cart-totals-right text-right grand-total">
