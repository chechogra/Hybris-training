	...
	<div class="rating-stars js-ratingCalc ${starsClass}" data-rating='{"rating":"${product.averageRating}","total":5}' >
		<div class="greyStars">
			<c:forEach  begin="1" end="5">
				<svg class="icon icon-star js-ratingIcon"><use xlink:href="${symbolDefsPath}#icon-star"></use></svg>
			</c:forEach>
		</div>
		<div class="greenStars js-greenStars">
			<c:forEach  begin="1" end="5">
				<svg class="icon icon-star-full js-ratingIcon"><use xlink:href="${symbolDefsPath}#icon-star-full"></use></svg>
			</c:forEach>
		</div>
	</div>
