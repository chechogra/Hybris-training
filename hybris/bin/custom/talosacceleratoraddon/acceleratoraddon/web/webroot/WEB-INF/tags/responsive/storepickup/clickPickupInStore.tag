...
<ycommerce:testId code="pickupInStoreButton">
    <c:choose>
        ...
        <c:when test="${searchResultsPage}">
            <button class="btn btn-default btn-block js-pickup-in-store-button glyphicon glyphicon-map-marker" disabled="disabled"
                    id="product_${fn:escapeXml(product.code)}${entryNumber}" type="button submit"
                    ...
                    data-productname="${fn:escapeXml(product.name)}" data-cartpage="false" data-entryNumber="0"
        ...
        <c:otherwise>
            <button class="btn btn-default btn-block js-pickup-in-store-button glyphicon-map-marker btn-icon" disabled="disabled"
                    id="product_${fn:escapeXml(product.code)}${entryNumber}" type="submit"
                    data-productavailable="${product.availableForPickup}"
                    data-productcart='${fn:escapeXml(product.price.formattedValue)}'
                    data-productcart-variants='{
					<c:if test="${not empty product.baseOptions[0].selected.variantOptionQualifiers}">
						<c:forEach var="variant" items="${product.baseOptions[0].selected.variantOptionQualifiers}" varStatus="productcartVariantsNumber">
